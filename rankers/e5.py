...
class E5SmallRanker(BaseRanker):
    name = "e5-small-v2"
    label = "E5 Small v2"
    ...
    def __init__(self, corpus, model_name=None):
        ...
        cache_base = os.path.join(cache_dir, f"e5_{model_slug}_{fingerprint}")
        ...
        texts = [self._format_passage(doc["text"]) for doc in corpus]
    ...
    def _format_query(self, text):
        return f"query: {text}"

    def _format_passage(self, text):
        return f"passage: {text}"

    def rank(self, query, top_k=10):
        query_vec = self._model.encode(
            [self._format_query(query)], normalize_embeddings=True
        )[0]
